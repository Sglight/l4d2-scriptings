...
public void OnPluginStart()
{
	...
	g_cvDASConVars[1] = CreateConVar("das_announcedifficulty", "0", "Announce the difficulty when it is changed?\n(0: OFF)\n(1: ON)");
	//g_cvDASConVars[2] = FindConVar("z_difficulty");
	g_cvDASConVars[2] = CreateConVar("das_fakedifficulty", "1", "Fake Difficulty.");
	g_cvDASConVars[3] = CreateConVar("das_disabledgamemodes", "", "Disable the Difficulty Adjustment System in these game modes.\nGame mode limit: 64\nCharacter limit for each game mode: 32\n(Empty: None)\n(Not empty: Disabled in these game modes.)");
	g_cvDASConVars[4] = CreateConVar("das_easydifficulty", "1", "Minimum players required for Easy.");
	g_cvDASConVars[5] = CreateConVar("das_enabledgamemodes", "coop,versus,realism,mutation1,mutation12", "Enable the Difficulty Adjustment System in these game modes.\nGame mode limit: 64\nCharacter limit for each game mode: 32\n(Empty: None)\n(Not empty: Enabled in these game modes.)");
	...
	// g_cvDASConVars[8].AddChangeHook(vDASDifficultyCvar);
	g_cvDASConVars[10] = CreateConVar("das_suffix", "", "Add suffix to cfg for more than one mode.");
...
public void OnConfigsExecuted()
{
	...
	char sDifficulty[32];
	char sSuffix[32];
	GetConVarString(g_cvDASConVars[10], sSuffix, sizeof(sSuffix));
	for (int iDifficulty = 0; iDifficulty <= 3; iDifficulty++)
	{
		...
		Format(sDifficulty, sizeof(sDifficulty),"%s%s", sDifficulty, sSuffix);
	...
	if (iFakeDifficulty != 0 && IsMapValid(sMap))
	{
		...
		switch (iFakeDifficulty)
		{
				case 1: sDifficultyConfig = "easy";
				case 2: sDifficultyConfig = "normal";
				case 3: sDifficultyConfig = "hard";
				case 4: sDifficultyConfig = "impossible";
		}
		Format(sDifficultyConfig, sizeof(sDifficulty), "%s%s", sDifficultyConfig, sSuffix);
...
public void vDASDifficultyCvar(ConVar convar, const char[] oldvalue, const char[] newvalue)
{
	...
	Format(sDifficultyConfig, sizeof(sDifficultyConfig), "%s%s", sDifficultyConfig, sSuffix);
	Format(sDifficultyConfig, sizeof(sDifficultyConfig), "cfg/sourcemod/difficulty_adjustment_system/%s.cfg", sDifficultyConfig);
	if (FileExists(sDifficultyConfig, true))
	{
		strcopy(sDifficultyConfig, sizeof(sDifficultyConfig), sDifficultyConfig[4]);
...
public Action tTimerUpdatePlayerCount(Handle timer)
{
	...
	//PrintToChatAll("iPlayerCount = %d", iPlayerCount);
	if (!g_bDASBools[0] && (iPlayerCount == iEasy || (iPlayerCount > iEasy && iPlayerCount < iNormal)))
	{
		//g_cvDASConVars[2].SetString("easy");
		g_cvDASConVars[2].SetInt(1);
		//SetConVarInt(g_cvDASConVars[2], 1);
		g_bDASBools[0] = true;
		g_bDASBools[1] = false;
		g_bDASBools[2] = false;
		g_bDASBools[3] = false;
	...
	else if (!g_bDASBools[1] && (iPlayerCount == iNormal || (iPlayerCount > iNormal && iPlayerCount < iAdvanced)))
	{
		//g_cvDASConVars[2].SetString("normal");
		g_cvDASConVars[2].SetInt(2);
		//SetConVarInt(g_cvDASConVars[2], 2);
		g_bDASBools[0] = false;
		g_bDASBools[1] = true;
		g_bDASBools[2] = false;
		g_bDASBools[3] = false;
	...
	else if (!g_bDASBools[2] && (iPlayerCount == iAdvanced || (iPlayerCount > iAdvanced && iPlayerCount < iExpert)))
	{
		//g_cvDASConVars[2].SetString("hard");
		g_cvDASConVars[2].SetInt(3);
		//SetConVarInt(g_cvDASConVars[2], 3);
		g_bDASBools[0] = false;
		g_bDASBools[1] = false;
		g_bDASBools[2] = true;
		g_bDASBools[3] = false;
	...
	else if (!g_bDASBools[3] && (iPlayerCount == iExpert || (iPlayerCount > iExpert && iPlayerCount < 5)))
	{
		//g_cvDASConVars[2].SetString("impossible");
		g_cvDASConVars[2].SetInt(4);
		//SetConVarInt(g_cvDASConVars[2], 4);
		g_bDASBools[0] = false;
		g_bDASBools[1] = false;
		g_bDASBools[2] = false;
		g_bDASBools[3] = true;
...
public void OnClientChangeTeam(Event event, const char[] name, bool dontBroadcast)
{
