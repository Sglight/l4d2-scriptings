...
public void OnPluginStart()
{
	...
	RegConsoleCmd("sm_ji", Infected_Cmd);
...
public Action Infected_Cmd(int client, int args)
{
	if (!isClientValid(client)) return Plugin_Handled;
	if (gameStarted && !IsInfectedTeamFull())
	{
		return Plugin_Stop;
	}
...
public Action Event_PlayerTeam(Handle event, const char[] name, bool dontBroadcast)
{
	...
	if (disconnect) return Plugin_Handled;

	if (isClientValid(client) && newteam == TEAM_INFECTED && IsInfectedTeamFull())
	{
		PrintHintText(client, "感染者团队已满！");
		ChangeClientTeam(client, TEAM_SPECTATORS);
	}
	return Plugin_Handled;
}
...
public Action Event_RoundStart(Handle event, const char[] name, bool dontBroadcast)
{
	...
	/* while (Survivors() > 0 && TotalSurvivors() < MAXSURVIVORS)
	{
		SpawnFakeClientAndTeleport();
	} */
	ClearInfectedTeam();
...
public void OnGameFrame() {
	for (int i = 1; i <= MaxClients; ++i)
	{
		if ((isClientValid(i) && GetClientTeam(i) == TEAM_INFECTED) || !gameStarted || IsInfectedTeamFull()) // 检查感染者
		{
			// if (gameStarted || Survivors() > MAXSURVIVORS)
			// {
			ChangeClientTeam(client, TEAM_SPECTATORS);
			// }
			// else
...
//                 Join Infected                 //
///////////////////////////////////////////////////
void ClearInfectedTeam()
{
	for (int i = 1; i <= MaxClients; i++)
	{
		if (isClientValid(i) && GetClientTeam(i) == TEAM_INFECTED)
		{
...
bool IsInfectedTeamFull()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		if (isClientValid(i) && GetClientTeam(i) == TEAM_INFECTED) // 排除未加入的和AI特感
		{
			count++;
		}
	}
