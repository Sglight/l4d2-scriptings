...
public void OnClientDisconnect(int cli)
{
	g_bShowProp[cli] = false;
}
...
void CheckClient(int cli)
{
	if( g_bShowProp[cli] == true )
	{
		g_bShowProp[cli] = false;
...
public void Event_PlayerSpawn(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( cli > 0 )
	{
		if( !IsFakeClient(cli) && GetClientTeam(cli) == 3 && GetEntProp(cli, Prop_Send, "m_zombieClass") == 6 )
		{
			g_bShowProp[cli] = true;
		}
		else
		{
			CheckClient(cli);
...
public void Event_RoundEnd(Handle h_Event, const char[] s_Name, bool b_DontBroadcast)
{
	ResetPlugin(true);
}
